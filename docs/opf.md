...
- ``optim_log_probabilistic_opf.log``
- ``optim_log_stochastic_opf.log``
- ``optim_log_cc-opf.log``.
...
The results file contains multiple sheets, starting with the ``main`` sheet, which provides a summary of the entire Excel file.
Additional sheets include detailed information such as the power flow through each line and other relevant network parameters.
...
| Market Cost Info  |   float   | Info about the market per market scenario and period                                                                                             |
|      Voltage      |   float   | **Voltage** results (phase and magnitude) per market and operational scenario and period                                                         |
|    Consumption    |   float   | **Consumption** data (reactive and active power) per market and operational scenario and period                                                  |
|    Generation     |   float   | **Generation** results (reactive and active power) per market and operational scenario and period                                                |
|  Capacitor Banks  |   float   | Contains the Step and reactive power **provided** of each **Capacitor Bank** per market and operational scenario and period                      |
|     Reactors      |   float   | Contains the Step and reactive power **consumed** of each **Reactor** per market and operational scenario and period                             |
|   Branch Losses   |   float   | **Losses** in each branch per market and operational scenario and period                                                                         |
|   Transformers    |   float   | Contains the Tap position and Ratio of each **Transformer** per market and operational scenario and period                                       |
|  Branch Loading   |   float   | Refers to how much of the **branch capacity** is being used in a certain direction per market and operational scenario and period                |
|    Power Flows    |   float   | **Power Flow** transit results in each branch per market and operational scenario and period                                                     |
|  Energy Storage   |   float   | **Energy Storage units** results (e.g, P [MW], SoC [MWh] and SoC [%]) through out every period for each market and operational scenario          |
| Relaxation Slacks |   float   | **Relaxation Slacks** used and their respected values per market and operational scenario and period                                             |